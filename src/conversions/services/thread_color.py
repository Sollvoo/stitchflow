...
_PALETTES_SEARCH_PATHS = [
    ...
    Path.home() / 'Library' / 'Application Support' / 'org.inkscape.Inkscape' / 'config'
    / 'inkscape' / 'extensions' / 'inkstitch' / 'inkstitch.app' / 'Contents' / 'Resources' / 'palettes',
...
def _load_palette() -> list[tuple[int, int, int, str]] | None:
    ...
    if path is None:
        logger.warning('[thread_color] Palette Brother Embroidery introuvable — snap désactivé.')
        return None
    _BROTHER_PALETTE = _parse_gpl(path)
    logger.info('[thread_color] Palette Brother chargée : %d fils depuis %s', len(_BROTHER_PALETTE), path.name)
    return _BROTHER_PALETTE
...
def snap_svg_colors_to_brother_palette(svg_path: Path) -> None:
    ...
    if changed:
        ...
        logger.info('[thread_color] %d couleur(s) snappée(s) vers palette Brother dans %s',
                    sum(1 for k, v in replacements.items() if k != v), svg_path.name)
