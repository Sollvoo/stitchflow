...
def remove_background(path: Path) -> Path:
    ...
    try:
        ...
        logger.warning(
            "rembg indisponible ou erreur (%s), fallback seuillage Pillow.", exc
        )
...
def _detect_image_type(path: Path, n_colors: int) -> str:
    ...
    with Image.open(path) as img:
        ...
        if variance <= _ENTROPY_LOGO_MAX:
            logger.debug(
                "[routing] entropie=%.1f → logo (seuil≤%.0f)",
                variance,
                _ENTROPY_LOGO_MAX,
            )
            return "logo"
        if variance >= _ENTROPY_PHOTO_MIN:
            logger.debug(
                "[routing] entropie=%.1f → photo (seuil≥%.0f)",
                variance,
                _ENTROPY_PHOTO_MIN,
            )
            return "photo"
        ...
        result = (
            "logo"
            if img.convert("RGB").getcolors(maxcolors=max_colors) is not None
            else "photo"
        )
...
def _consolidate_svg_colors(svg_path: Path, n_colors: int) -> None:
    ...
    if changed:
        ...
        logger.info(
            "[vtracer] %d couleurs → %d clusters (max %d)",
            len(fill_counts),
            unique_after,
            n_colors,
        )
...
def _quantize_to_n_colors(path: Path, n_colors: int) -> Path:
    ...
    with Image.open(path) as img:
        ...
        quantized = rgb.quantize(
            n_colors, method=Image.Quantize.FASTOCTREE, dither=Image.Dither.NONE
        )
...
def vectorize_to_svg(png_path: Path, n_colors: int = 6) -> Path:
    ...
    try:
        ...
        if fine_details:
            logger.info(
                "[routing] détails fins détectés dans %s — paramètres précision max",
                png_path.name,
            )

        image_type = _detect_image_type(flattened_path, n_colors)
        logger.info(
            "[routing] %s → %s (n_colors=%d)", png_path.name, image_type, n_colors
        )

        if image_type == "logo":
            # Logos : potrace en priorité, couleurs extraites pixel par pixel sans quantification.
            try:
                return _vectorize_potrace(
                    flattened_path, svg_path, n_colors, fine_details=fine_details
                )
            except RuntimeError as potrace_err:
                logger.warning(
                    "[routing] potrace échoué pour logo (%s), fallback VTracer.",
                    potrace_err,
                )
        ...
        logger.warning(
            "VTracer Python échoué (code %d%s), tentative potrace.",
            result.returncode,
        ...
        try:
            return _vectorize_potrace(
                quantized_path, svg_path, n_colors, fine_details=fine_details
            )
        except RuntimeError as potrace_err:
...
def _extract_potrace_paths(svg_path: Path) -> list[str]:
    ...
    try:
        ...
        return []
    return [
        el.get("d") for el in root.iter() if el.tag.endswith("path") and el.get("d")
    ]


def _build_svg(
    paths_with_colors: list[tuple[list[str], str]], width: int, height: int
) -> str:
    """
...
def _cluster_exact_colors(
    ...
    Retourne les N clusters dominants non-blancs : (couleur_centre, couleurs_membres).
    """
    import math
...
def _vectorize_potrace(
    png_path: Path, svg_path: Path, n_colors: int = 6, fine_details: bool = False
) -> Path:
    """
    ...
    logger.info(
        "[DEBUG potrace] n_colors=%d, image=%dx%d, unique_colors=%s",
        n_colors,
        width,
        height,
        (
            f"{len(unique_colors)} couleurs exactes"
            if unique_colors is not None
            else "trop nombreuses → MEDIANCUT"
        ),
    )
        ...
        if unique_colors is not None:
            ...
            for idx, (center, color_set) in enumerate(clusters):
                ...
                if result.returncode != 0 or not potrace_svg.exists():
                    logger.warning(
                        "[DEBUG potrace] potrace échoué pour %s (code %d, stderr=%s)",
                        hex_color,
                        result.returncode,
                ...
                logger.info(
                    "[DEBUG potrace] paths potrace pour %s : %d path(s)",
                    hex_color,
                    len(d_paths),
                )
                if d_paths:
            ...
            quantized = rgb.quantize(
                colors=n_colors + 1, method=Image.Quantize.FASTOCTREE
            )
            ...
            for color_idx, _ in counts.most_common():
                ...
                if result.returncode != 0 or not potrace_svg.exists():
                    logger.warning(
                        "potrace échoué pour couleur %s (code %d)",
                        hex_color,
                        result.returncode,
                    )
                    continue
    ...
    logger.info(
        "Vectorisation potrace : %d couleur(s) → %s (%d KB)",
        len(paths_with_colors),
        svg_path.name,
        svg_size_kb,
    )
...
def _simplify_svg_nodes(svg_path: Path) -> None:
    ...
    try:
        result = subprocess.run(
            [
                inkscape,
                "--actions=select-all;path-simplify;export-do",
                f"--export-filename={svg_path}",
                str(svg_path),
            ],
            capture_output=True,
        ...
        if result.returncode == 0 and svg_path.exists() and svg_path.stat().st_size > 0:
            ...
            logger.info(
                "[DEBUG potrace] SVG simplifié par Inkscape : %d KB → %d KB",
                before_kb,
                after_kb,
            )
        else:
            logger.warning(
                "[DEBUG potrace] Inkscape simplify échoué (code %d)", result.returncode
            )
