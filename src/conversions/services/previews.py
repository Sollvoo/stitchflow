...
def _filter_pes_v1_color_breaks(threadlist: list) -> tuple[list, bool]:
    ...
    for i, t in enumerate(threadlist):
        if _is_near_white_thread(t.color):
            ...
            next_real = i < len(threadlist) - 1 and not _is_near_white_thread(threadlist[i + 1].color)
...
def _score_color_fidelity(
    ...
    if score >= 85:
        msg = f'Couleurs fidèles (Δ Lab moyen {mean_dist:.1f}, {len(pes_colors)}/{len(svg_colors)} fil(s))'
    elif score >= 60:
        msg = f'Fidélité correcte (Δ Lab {mean_dist:.1f}, {len(pes_colors)}/{len(svg_colors)} fil(s))'
    elif score >= 30:
        msg = f'Dérive couleurs notable (Δ Lab {mean_dist:.1f}, {len(pes_colors)}/{len(svg_colors)} fil(s))'
    else:
        msg = f'Mauvaise fidélité couleurs (Δ Lab {mean_dist:.1f}, {len(pes_colors)}/{len(svg_colors)} fil(s) — vectorisation partielle ?)'
    return score, msg
...
def _compute_quality_score(
    ...
    if thread_count == 0:
        ...
        t_score = 25
        t_msg = f'{thread_count} fils — dépasse les 10 aiguilles, re-enfilage nécessaire'
    else:
        t_score = 0
    ...
    if width_mm > 0 and height_mm > 0:
        if width_mm <= 360 and height_mm <= 200:
            if width_mm >= 20 and height_mm >= 5:
                ...
                d_score = 35
                d_msg = f'{width_mm:.0f}×{height_mm:.0f} mm — très petit, détails risquent d\'être illisibles'
    ...
    if jump_ratio < 0.005:
        ...
        j_score = 45
        j_msg = f'{jump_count} sauts ({jump_ratio*100:.1f}%) — nombreux, risque casse fil'
    else:
        j_score = 10
    ...
    if source_svg_path and source_svg_path.exists():
        cov_score, cov_msg = _score_vectorization_coverage(source_svg_path, n_colors_requested)
    else:
    ...
    details = {
        'threads': {
            'score': t_score, 'message': t_msg, 'weight': 18,
            'raw_value': thread_count,
            'thresholds': {'ideal': 7, 'max_machine': 10, 'hard_limit': 15},
        },
        'stitches': {
            'score': s_score, 'message': s_msg, 'weight': 18,
            'raw_value': stitch_count,
            'thresholds': {'min_simple': 500, 'min_pro': 2000, 'max_recommended': 500000},
        },
        'dimensions': {
            'score': d_score, 'message': d_msg, 'weight': 14,
            'raw_value': {'width_mm': round(width_mm, 1), 'height_mm': round(height_mm, 1)},
            'thresholds': {'max_w': 360, 'max_h': 200},
        },
        'jumps': {
            'score': j_score, 'message': j_msg, 'weight': 10,
            'raw_value': {'jump_count': jump_count, 'ratio_pct': round(jump_ratio * 100, 2)},
        ...
        'coverage': {
            'score': cov_score, 'message': cov_msg, 'weight': 12,
            'raw_value': None,
        },
    }
            ...
            if cmd == pyembroidery.STITCH:
                if last_x is not None:
                    draw.line([(last_x, last_y), (sx, sy)],
                              fill=_thread_color(color_idx), width=line_w)
                last_x, last_y = sx, sy
...
def extract_pes_metadata(
    ...
    try:
        ...
        if was_filtered:
            logger.info('[pes] %d fils réels extraits (sur %d entries PES v1 avec COLOR_BREAK blancs)',
                        len(threadlist), len(raw_threadlist))
        color_changes = len(threadlist)

        thread_colors = [
            {'hex': f'#{t.color:06X}', 'name': t.description or ''}
            for t in threadlist
        ]
